...
-- tick (autostart stat), owns the DEFERRED once-per-session pass that used to run
-- at Lumen boot, and runs a final pass before exit. loop.run is an infinite loop,
-- so we assert on the source rather than executing it.
...
ok(src:find("deskcover.new_initial", 1, true),
   "loop.lua owns the deferred once-per-session pass")
ok(src:find("dc_initial:due(now)", 1, true),
   "the deferred pass is gated on the tick clock, not on boot")
